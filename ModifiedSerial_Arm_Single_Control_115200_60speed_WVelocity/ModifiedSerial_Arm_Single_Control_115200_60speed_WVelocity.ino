/***************************
* Modified Arduino Sketch to control a Dynamixel robot arm
* ME 457/557  
* 3/11/2020
* Drew Wendeborn
* dw8@pdx.edu
...
uint16_t joints[joint_num];                 // Two bytes for each desired joint number
uint16_t thetas[joint_num];                 // Array to store the current joint angles
uint16_t grippers[gripper_num];
uint16_t thetadots[joint_num];
...
void driveJoints(){
   ...
   for (int i = 0; i < joint_num; i++){       //Could use the scan function here instead
         ...
         dxl_wb.goalVelocity(i, (int32_t)thetadots[i]);
...
void driveJointswithVelocity(){
   ...
   for(int i = 0; i <= joint_num ; i++){
          uint8_t hi = Serial.read();
          uint8_t lo = Serial.read();
          uint16_t thetadot = hi << 8;           // Bit shift the hi byte to the highByte position in the variable
          thetadot |= lo;
          thetadots[i] = thetadot-10; // Subtract 10 for reserved numbers corresponding to serial codes
        }
        delay(15000);
  for(int i = 0; i <= joint_num ; i++){
    
              Serial.print(9);
              Serial.print(highByte(thetadots[i]));
              Serial.print(lowByte(thetadots[i]));   
  }
