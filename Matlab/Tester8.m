...
while key ~= 13  % Enter
    ...
    %% Discretized Path and Move
        steps = 5; % How many steps?
        deeposlist = zeros(100,length(desiredeepos));
        for i = 1:length(home)
            deeposlist(:,i) = linspace(currenteepos(i),desiredeepos(i),steps);
        end
        
        dthetalist = EE2Theta(deeposlist,Slist,M,ew,ev);
        RInt(dthetalist);
        key = getkey;
end
