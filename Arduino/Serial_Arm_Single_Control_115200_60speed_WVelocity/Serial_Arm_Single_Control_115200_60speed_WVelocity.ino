...
void driveJointswithVelocity(){
   ...
   for (int i = 0; i < joint_num; i++){       //Could use the scan function here instead
         ...
         dxl_wb.goalVelocity(i, (int32_t)thetadot[i]);
...
void driveJoints(){
   ...
   for(int i = 0; i <= joint_num ; i++){
          ...
          thetadot |= lo;
          thetadot[i] = thetadot-10;               // Subtract 10 for reserved numbers corresponding to serial codes
        }
        

   for (int i = 0; i < joint_num; i++){       //Could use the scan function here instead
         dxl_wb.ping(i, &model_number);
         dxl_wb.goalVelocity(i, (int32_t)velocity[i]);
         dxl_wb.goalPosition(i,(int32_t)joints[i]);
         delay(arm_delay);
    }  
}
